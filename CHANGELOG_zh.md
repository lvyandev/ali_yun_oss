...
### 📦 依赖
- 将 `xml` 依赖约束放宽为 `>=6.6.1 <8.0.0`，允许仍解析到 `xml` 6.x 的项目使用本包，同时继续兼容 `xml` 7.x。

### 🔄 兼容性
- 向后兼容：仅调整依赖约束，不包含 API 或行为变更。
...
### ✨ 增强
- 日志拦截器：默认启用“分组输出”，一次请求/响应/错误仅打印一条多行日志，显著降噪。
- 新增参数 `groupPrint`（默认 true），设置 `groupPrint: false` 可回退为逐行输出。
- 默认 `maxLogLength` 从 500 提升到 4000，减少日志内容被截断的情况。
- 继续保留敏感头脱敏与二进制体仅打印长度的策略。
- 默认日志方法恢复为 `print`。

### 🔄 兼容性
- 向后兼容：仅新增/默认行为调整，无破坏性变更。
